...
function getDisputeAccess(booking, now = new Date()) {
  ...
  if (status === "completed" && completedReference) {
    const expiresAt = new Date(
      completedReference.getTime() + DISPUTE_WINDOW_MS,
    );
    ...
    return {
      allowed: false,
      message:
        "Disputes for completed bookings must be raised within 24 hours of trip completion",
      mode: "expired",
      expiresAt,
    };
  }
  ...
  return {
    allowed: false,
    message:
      "You can report an issue only during an ongoing trip or raise a dispute within 24 hours after completion",
    mode: "unavailable",
  };
}
...
async function getPendingUsers(req, res, next) {
  try {
    ...
    const [users, total, pendingTotal, verifiedToday] = await Promise.all([
      User.find(filter)
        .select(
          "name email role phone createdAt isVerified image citizenshipNo licenseNumber kycData",
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(Number(limit))
...
async function getOwnerEarningsSummary(req, res, next) {
  try {
    ...
    const vehicleMap = new Map(
      vehicleAgg.map((row) => [String(row._id), row.vehicles || 0]),
    );
    ...
    const rows = owners.map((owner) => {
      ...
      const bookingStats = bookingMap.get(ownerId) || {
        totalBookings: 0,
        grossEarnings: 0,
      };
  ...
  try {
    ...
    const [
      ...
      PayoutRequest.find({ owner: ownerObjectId, status: "pending" })
        .select(
          "amount status paymentMethod payoutDetails createdAt updatedAt note",
        )
        .sort({ createdAt: -1 })
        .lean(),
    ]);
...
async function updatePayoutRequestStatus(req, res, next) {
  try {
    ...
    if (["paid", "rejected"].includes(payoutRequest.status)) {
      return res
        .status(409)
        .json({ message: `Cannot change status from ${payoutRequest.status}` });
    }

    if (payoutRequest.status !== "pending") {
      return res.status(409).json({
        message: `Cannot mark ${payoutRequest.status} request as ${status}`,
      });
    }

    payoutRequest.status = status;
    payoutRequest.note = String(note || "").trim();
    payoutRequest.processedAt = ["paid", "rejected"].includes(status)
      ? new Date()
...
async function createDispute(req, res, next) {
  try {
    ...
    const { bookingId, reason, description, incidentAt, mode: requestedMode } = req.body;
    const mode = String(requestedMode || 'issue').toLowerCase();
    if (!reporterId)
    ...
    if (!['issue', 'dispute'].includes(mode)) {
      return res.status(400).json({ message: 'Invalid mode. Allowed values: issue, dispute' });
    }

    if (!["renter", "owner"].includes(String(reporterRole))) {
      return res
        .status(403)
        .json({ message: "Only renter or owner can create disputes" });
    }

    const booking = await Booking.findById(bookingId)
      .select("renter owner status startDate endDate completedAt updatedAt")
      .lean();
    ...
    ].includes(String(reporterId));
    if (!isParticipant) {
      return res
        .status(403)
        .json({ message: "Not allowed to create dispute for this booking" });
    }
    ...
    // - 'issue' can only be created while booking is ongoing (disputeAccess.mode === 'issue') and may be created multiple times
    // - 'dispute' can only be created for completed bookings within the dispute window (disputeAccess.mode === 'dispute') and only once
    if (mode === 'issue' && disputeAccess.mode !== 'issue') {
      return res.status(400).json({ message: 'Issues can only be created during an ongoing trip' });
    }

    if (mode === 'dispute') {
      if (disputeAccess.mode !== 'dispute') {
        return res.status(400).json({ message: 'Disputes can only be created for completed bookings within the dispute window' });
      }

      // Prevent creating more than one completed-booking dispute (mode: 'dispute')
      const existingCompletedBookingDispute = await Dispute.findOne({ bookingId, mode: 'dispute' }).select('_id reporterId status').lean();
      if (existingCompletedBookingDispute) {
        return res.status(409).json({ message: 'A dispute has already been created for this completed booking' });
      }

      // Also block if an active dispute (mode: 'dispute') exists
      const existingOpenDispute = await Dispute.findOne({ bookingId, mode: 'dispute', status: { $in: ACTIVE_DISPUTE_STATUSES } }).lean();
    ...
    const openedMessageParts = [
      ...
      parsedIncidentAt
        ? `Incident time: ${parsedIncidentAt.toISOString()}`
        : null,
    ...
    const recipients = [renterUser, ownerUser].filter(
    ...
    res.status(201).json({
      ok: true,
      dispute: normalizeDispute(dispute),
...
async function getDisputes(req, res, next) {
  try {
    ...
    const filter = {};
    if (status && DISPUTE_STATUSES.includes(String(status)))
      filter.status = String(status);
    if (reason && String(reason).trim())
      filter.reason = new RegExp(String(reason).trim(), "i");
    ...
    if (search && String(search).trim()) {
      ...
      const matchingBookings = await Booking.find({
        $or: [{ renter: { $in: userIds } }, { owner: { $in: userIds } }],
      })
        .select("_id")
        .lean();
      const bookingIds = matchingBookings.map((b) => b._id);

      const byObjectId = /^[a-f\d]{24}$/i.test(String(search).trim())
        ? [String(search).trim()]
        : [];
      filter.$or = [
        { _id: { $in: byObjectId } },
  ...
  try {
    ...
    const dispute = await Dispute.findById(id)
      .populate({
        path: "bookingId",
        select:
          "startDate endDate totalPrice status renter owner vehicle payment preStartPhotos",
        populate: [
          { path: "renter", select: "name email phone image role" },
          { path: "owner", select: "name email phone image role" },
          {
            path: "vehicle",
            select: "brand model registrationNumber type photos status",
          },
        ],
      })
    ...
    const timeline = (dispute.timeline || []).map((event) => ({
      ...
      actorName:
        event.actorName || (event.actorId && event.actorId.name) || "System",
...
async function getDisputeByBooking(req, res, next) {
  try {
    ...
    // Return only completed-booking disputes (mode: 'dispute') so callers
    // can reliably detect a filed dispute for a completed booking.
    const dispute = await Dispute.findOne({ bookingId, mode: 'dispute' })
      .sort({ createdAt: -1 })
      .populate({
        path: "bookingId",
        select:
          "startDate endDate totalPrice status renter owner vehicle payment completedAt updatedAt",
        populate: [
          { path: "renter", select: "name email phone image role" },
          { path: "owner", select: "name email phone image role" },
          {
    ...
    if (!dispute) {
      return res.json({ ok: true, dispute: null });
    }
    ...
    const isParticipant =
      booking &&
      [
        String(booking.renter?._id || booking.renter),
        String(booking.owner?._id || booking.owner),
      ].includes(String(authUserId));
...
async function resolveDispute(req, res, next) {
  try {
    ...
    const dispute = await Dispute.findById(id).populate(
      "bookingId",
      "status renter owner vehicle",
    );
    if (!dispute) return res.status(404).json({ message: "Dispute not found" });

    if (!DISPUTE_STATUSES.includes(status)) {
      return res.status(400).json({
        message: `Invalid status. Allowed: ${DISPUTE_STATUSES.join(", ")}`,
      });
    }

    if (CLOSED_DISPUTE_STATUSES.includes(dispute.status)) {
      return res.status(409).json({
        message: "This dispute is already closed and can no longer be updated",
      });
    }

    const resolver = resolverId
      ? await User.findById(resolverId).select("name").lean()
      : null;
    ...
    if (action === "cancel") {
      await Booking.findByIdAndUpdate(dispute.bookingId, {
        status: "cancelled",
      });
    }
    ...
    const recipients = [
      updated?.bookingId?.renter,
      updated?.bookingId?.owner,
    ].filter((participant) => participant?.email);
    const vehicleName =
      [updated?.bookingId?.vehicle?.brand, updated?.bookingId?.vehicle?.model]
        .filter(Boolean)
        .join(" ") || "Booked Vehicle";
