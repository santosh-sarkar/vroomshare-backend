 ...
 * GET /chat/conversations
 * Returns all conversations for the logged-in user, populated with
 * the other participant's name, avatar (profile pic placeholder) and online status.
 */
async function getConversations(req, res) {
  const userId = req.user.sub;
  ...
  const result = conversations.map((c) => {
    ...
    return {
      ...
      name: other?.name ?? 'Unknown',
      img: other?.image?.profile ?? null,
      title: '', // caller can enrich with booking/vehicle title if needed
      text: c.lastMessage?.text ?? '',
      time: c.lastMessage?.sentAt
        ? formatTime(c.lastMessage.sentAt)
        : formatTime(c.updatedAt),
      online: false, // real online status managed by Socket.io
      otherUserId: other?._id,
    };
  });
 ...
 * POST /chat/conversations
 * Body: { recipientId }
 * Creates a new conversation if one doesn't already exist between the two users,
